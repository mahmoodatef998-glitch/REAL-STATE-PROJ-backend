...
export class UsageService {
     ...
     */
    static async increment(companyId: number, resourceType: string): Promise<Usage | null> {
        if (!companyId) {
            return null;
        }
     ...
     */
    static async decrement(companyId: number, resourceType: string): Promise<Usage | null> {
        if (!companyId) {
            return null;
        }
     ...
     */
    static async checkLimit(companyId: number, resourceType: string, plan: any): Promise<any> {
        if (!companyId || !plan) {
        ...
        try {
            ...
            const limitFieldMap: { [key: string]: string } = {
     ...
     */
    static async getCurrentUsage(companyId: number): Promise<any> {
        if (!companyId) {
            return {
                property: 0,
                broker: 0,
