...
import { Property as PrismaProperty, User as PrismaUser } from '@prisma/client';
...
export class Property {
    ...
    constructor(data: any) {
        ...
        this.area_sqft = data.area_sqft !== undefined ? data.area_sqft : data.areaSqft;
        ...
        this.images = Array.isArray(data.images)
            ? data.images
            : (typeof data.images === 'string' && data.images ? Property.safeParseJson(data.images) : []);
        this.features = Array.isArray(data.features)
            ? data.features
            : (typeof data.features === 'string' && data.features ? Property.safeParseJson(data.features) : []);
        this.owner_id = data.owner_id !== undefined ? data.owner_id : data.ownerId;
    ...
    static safeParseJson(text: string): any[] {
        try { return JSON.parse(text); } catch { return []; }
    }

    static serializeArray(value: any): string | null {
        if (Array.isArray(value)) return JSON.stringify(value);
        return value || null;
    }

    static async create(propertyData: any): Promise<Property> {
        const created = await prisma.property.create({
            data: {
                ...
                images: Property.serializeArray(propertyData.images),
    ...
    static async findById(id: number | string): Promise<Property | null> {
    ...
    static async getByOwner(ownerId: number | string): Promise<Property[]> {
        const rows = await prisma.property.findMany({
    ...
    async update(updateData: any): Promise<Property> {
        const updated = await prisma.property.update({
            ...
            data: {
                ...
                images: updateData.images !== undefined ? Property.serializeArray(updateData.images) : undefined,
                features: updateData.features !== undefined ? Property.serializeArray(updateData.features) : undefined,
