...
export class Plan {
    ...
    static async findById(id: number | string): Promise<Plan | null> {
    ...
    static async findByName(name: string): Promise<Plan | null> {
        const plan = await prisma.plan.findUnique({
            where: { name }
        });
    ...
    async update(updateData: any): Promise<Plan> {
        ...
        Object.assign(this, new Plan(updated));
        return this;
    }
