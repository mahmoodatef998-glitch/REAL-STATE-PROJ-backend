...
router.get('/', authenticateToken, requireRole(['admin', 'broker']), async (req: Request, res: Response) => {
    try {
        const filters: any = {
...
router.get('/notifications/count', authenticateToken, requireRole(['admin', 'broker']), async (req: Request, res: Response) => {
    try {
        let count = 0;
...
// Get lead by ID
router.get('/:id', authenticateToken, requireRole(['admin', 'broker']), async (req: Request, res: Response) => {
    try {
        const lead = await Lead.findById(req.params.id as string);
...
router.put('/:id', authenticateToken, requireRole(['admin', 'broker']), async (req: Request, res: Response) => {
    try {
        const lead = await Lead.findById(req.params.id as string);
...
router.get('/property/:propertyId', authenticateToken, requireRole(['admin', 'broker']), async (req: Request, res: Response) => {
    try {
        const { propertyId } = req.params;
        const leads = await Lead.getByProperty(propertyId as string);
...
router.get('/stats/overview', authenticateToken, requireRole(['admin', 'broker']), async (req: Request, res: Response) => {
    try {
        const filters: any = {};
