...
router.get('/stats/overview', authenticateToken, requireRole(['admin']), async (req: Request, res: Response, next: NextFunction) => {
    try {
        const allUsers = await User.getAll();
...
router.get('/:id', authenticateToken, requireRole(['admin']), async (req: Request, res: Response, next: NextFunction) => {
    try {
        const user = await User.findById(parseInt(req.params.id as string));
...
router.post('/:id/approve', authenticateToken, requireRole(['admin']), async (req: Request, res: Response, next: NextFunction) => {
    try {
        const user = await User.findById(parseInt(req.params.id as string));
...
router.post('/:id/reject', authenticateToken, requireRole(['admin']), async (req: Request, res: Response, next: NextFunction) => {
    try {
        const user = await User.findById(parseInt(req.params.id as string));
