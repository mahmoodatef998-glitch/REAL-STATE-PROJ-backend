...
// In this implementation, we allow all authenticated users to see companies list as it might be needed for dropdowns
// But typically maybe only admins should see full details.
// The original code allowed all authenticated users.
...
router.put('/:id', authenticateToken, requireRole(['admin']), async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const updatedCompany = await Company.findById(req.params.id as string);
