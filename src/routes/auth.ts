 ...
 */
async function issueRefreshToken(res: Response, userId: number) {
    ...
    try {
        ...
        try {
            ...
            return res.status(500).json({
                success: false,
                error: errorMessage,
                details: CONFIG.isDevelopment ? tokenError.message : undefined
            });
        }
    } catch (error: any) {
...
router.put('/change-password', authenticateToken, async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        // We fetch user again to get the password hash
        const user = await User.findById(req.user.id);
        if (!user) {
            return res.status(404).json({ success: false, error: 'User not found' });
        }
...
router.get('/verify', authenticateToken, async (req: Request, res: Response, next: NextFunction) => {
    try {
        if (!req.user) {
