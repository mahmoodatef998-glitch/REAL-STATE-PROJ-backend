 ...
 * Calculate monthly commissions and income for deals
 */

const { prisma } = require('../database/db');
const { getMonthDateRange } = require('./dateHelper');
 ...
 */
async function calculateMonthlyBrokerCommission(brokerId, year, month) {
  ...
  deals.forEach(deal => {
    ...
    const commission = parseFloat(deal.commissionValue || 0);
    const brokerShare = parseFloat(deal.brokerShare || 0);
  ...
  return {
    ...
    deals: deals.map(deal => ({
      ...
      dealValue: deal.dealValue || deal.salePrice,
 ...
 */
async function calculateMonthlyCompanyIncome(companyId, year, month) {
  const { startDate, endDate } = getMonthDateRange(year, month);
  ...
  deals.forEach(deal => {
    const dealValue = parseFloat(deal.dealValue || deal.salePrice || 0);
    const commission = parseFloat(deal.commissionValue || 0);
    const companyShare = parseFloat(deal.companyShare || 0);
    const brokerShare = parseFloat(deal.brokerShare || 0);
 ...
 * @returns {Promise<Array>} Array of monthly commission summaries
 */
async function getBrokerCommissionHistory(brokerId, months = 12) {
  const history = [];
  const now = new Date();
 ...
 * @param {number} companyId - Company ID (optional)
 * @param {number} months - Number of months to retrieve (default: 12)
 * @returns {Promise<Array>} Array of monthly income summaries
 */
async function getCompanyIncomeHistory(companyId, months = 12) {
  const history = [];
  const now = new Date();
 ...
 */
async function compareBrokerMonths(brokerId) {
  const now = new Date();
