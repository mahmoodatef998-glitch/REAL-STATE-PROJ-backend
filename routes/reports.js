 ...
 * Handles monthly commission and income reports for admin and brokers
 */
...
const { getCurrentMonthRange, parseMonthString, getLastNMonths } = require('../utils/dateHelper');
 ...
 */
router.get('/broker/monthly', authenticateToken, async (req, res, next) => {
  try {
    let { year, month, brokerId } = req.query;
 ...
 */
router.get('/broker/history', authenticateToken, async (req, res, next) => {
  try {
    let { brokerId, months } = req.query;
 ...
 */
router.get('/broker/compare', authenticateToken, async (req, res, next) => {
  try {
    let { brokerId } = req.query;
 ...
 */
router.get('/company/monthly', authenticateToken, requireRole(['admin']), async (req, res, next) => {
  try {
    let { year, month, companyId } = req.query;
 ...
 */
router.get('/company/history', authenticateToken, requireRole(['admin']), async (req, res, next) => {
  try {
    let { companyId, months } = req.query;
 ...
 */
router.get('/months', authenticateToken, async (req, res, next) => {
  try {
 ...
 */
router.get('/brokers/performance', authenticateToken, requireRole(['admin']), async (req, res, next) => {
  try {
    let { year, month, companyId } = req.query;
